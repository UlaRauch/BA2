...
\hspace*{-1.0cm} {\LARGE \textbf{Implementation of OAuth 2.1 and OpenID Connect in a
microservice-based prototype with Spring Boot 3 and Keycloak server \\}}
\vspace{0.2cm}
...
\begin{figure}[htbp]
	...
	\caption{Monolithic architecture vs. microservice-based architecture \cite{rauchAuthxmicroservicesurauchPdf2023} according to \cite{lewisMicroservices2014}}
	\label{fig:msa}
\end{figure}
...
\subsection{The authentication and authorization in the context of MSA}
\label{sub:auth-msa}
...
For the present research, a similar approach has been choosen by implementing the gateway with client functionality, but leaving the frontend application out of scope, which could be served by the gateway, like in the BFF Proxy, but also in a seperate webserver or in a native application.  This is also the reason why the term BFF is not used in this thesis. According to Newman \cite{SamNewmanBackends}, authentication or authorization functionality alone is not sufficient to consider the gateway a BFF. The implementation is described in detail in section \ref{chap:implementation}. The services beyond are resource servers, so that the communication between the API gateway and the next downstream service is secured via access tokens.
...
Security configuration for the gateway's endpoints can be added in the way that is shown in listing \ref{lst:securitywebfilterchain}. Because \texttt{/hellogateway} and \texttt{/hellotea/noauth} should remain open for testing purposes, this is taken care of with \texttt{permitAll()} before configuring all remaining endpoints as open for authenticated users only with \texttt{.authorizeExchange().anyExchange().authenticated()}. With \texttt{oauth2login()} the users will be authenticated so they can have access to the protected endpoints \cite{rozaWebClientOAuth2Support2019}. 
...
As already mentioned in chapter \ref{chap:background}, the gateway as a client keeps a session with the front-end application. Session management works with Spring Boot out of the box by default without explicit configuration \cite{AuthenticationPersistenceSession}.  The next section also shows how a connection is configured to be stateless. 
...
\label{sub:rbac-impl}
As an extension to the answer to the first research question, also different user roles are defined with Keycloak and checked at the resource server before responding to the request. This is a very minimalistic interpretation of RBAC (see section \ref{sub:rbca}.
...
\begin{lstlisting}[frame=lines, caption=Extraction of client roles and realm roles from Keycloak access token and conversion to granted authorities according to \cite{ch4mpAnswerUseKeycloak2022}. Simplified with added \texttt{ROLE\_} prefix., captionpos=b, label=lst:roles-authorities-converter, language=Java]
...
When the Tea service now recieves an access token from a user with the wrong role, it responds with the \texttt{403 Forbidden} code and a \texttt{WWW-Authenticate} header with the message that higher privileges are required (see figure \ref{fig:postman-higher-privileges}.

\begin{figure}[htbp]
	\centering
		\includegraphics[width=\linewidth]{images/postman-higher-privileges}
	\caption{postman-higher-privileges}
...
This section describes the implementation and test setup for the answer to the second research question presented in chapter \ref{chap:intro}, about the 
...
As already mentioned at the beginning of this chapter, this version was built three times, with the client at the gateway as described in section \ref{sec:high-level}, with the client as resource server and a fictional client at the front-end level, and without any security implementation. For the load testing, JMeter imitated the client in the sense that it had an access token that it could send to the gateway (as resource server).

For testing purposes, an endpoint at the respective gateway was used that forwards the request and the answer to and from the Tea service. When the gateway is a client, it has to ckeck the session cookie, find the stored access token and append it to the request before routing it to the Tea service. When the gateway is a resource server, it has to validate the recieved access token. In both scenarios, the Tea service is a resource server as described in section \ref{sub:resource-server}. For authentication in the first case, user login was first triggered via the browser so that the client would send a session cookie to the browser and store the user's access token. This session cookie was extracted and sent with each request to the client-gateway. In the second case, the gateway as resource server expects an access token, so this token was generated with Postman and sent with each request.
...
The network traffic from the access the protected resource (the list of available Teas) via the gateway in it's client function at \texttt{http:localhost:8181/listteas} and a Firefox browser as user agent, was captured with Wireshark and is listed in the appendix \ref{appendix}.
...
The authorization/authentication request also contains the required parameters \texttt{response\_type}, \texttt{client\_id}, \texttt{redirect\_uri} and \texttt{scope} with \texttt{openid} as the value \cite{hardtOAuthAuthorizationFramework2023}, the last two parameters being required by OIDC \cite{sakimuraOpenIDConnectCore2014}. The \texttt{code\_challenge} parameter is missing. However, it is not required when the optional \texttt{nonce} parameter is used, which is the case here, but still recommended by the OAuth2 specification \cite{hardtOAuthAuthorizationFramework2023}. Also the recommended \texttt{state} parameter is present.

The token request is sent to Keycloak's token endpoint and contains again the \texttt{client\_id} and \texttt{grant\_type}, as required. The client authenticates during the token request with basic authentication. This is possible by definition of OAuth2\cite{hardtOAuthAuthorizationFramework2023} and it was a deliberate choice during the implementation. However, Spring Security and Keycloak both support other methods of client authentication, for example with a signed JWT \cite{AuthorizationServicesGuide}, \cite{ClientAuthenticationSupport}. The value in the authorization header is the base64-encoding of \texttt{client\_id:client\_secret}.
...
Another feature that was implemented but is is not part of OAuth2 or OIDC, is access control based on user roles. It was not the aim of this thesis to analyze the implementation in detail for compliance with the RBAC standard and it is clear, that the presented implementation is a very minimalistic interpretation of RBAC. Keycloak also supports other, more fine-grained methods of access control, like attribute-based access control (ACAB), but the limitation on roles seems sufficient as a proof of concept within the scope of this thesis. The implementation of ACAB can also be a suggestion for future work.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth} {|  >{\raggedright\arraybackslash}X  |   >{\raggedright\arraybackslash}X |} 
        ...
        \linebreak -client...}
		\linebreak
        \linebreak \texttt{Set-Cookie: KEYCLOAK\_SESSION=
        \linebreak teapot/9db57273-f45d-440f
        \linebreak -910e-8dc764c3bcb0/4165bd59
        \linebreak -749c-4ed3-a210-ecd4a6f928f5;
        \linebreak Version=1; Expires=Fri, 16-Jun-2023
        \linebreak 22:35:18 GMT; Max-Age=36000; 
        \linebreak Path=/realms/teapot/; 
        \linebreak SameSite=None; Secure}
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth} {|  >{\raggedright\arraybackslash}X  |   >{\raggedright\arraybackslash}X |} 
        ...
        \texttt{200 OK , JavaScript Object Notation (application/json)} \\
        \hline
        The gateway requests the JWKS from Keycloak
        &
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth} {|  >{\raggedright\arraybackslash}X |} 
        ...
        \hline
        Gateway refers user to it's own \texttt{/listteas} endpoint a second time \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth} {|  >{\raggedright\arraybackslash}X |} 
        ...
        \texttt{GET /listteas} \\
        \hline
        Browser sends new request to gateway's \texttt{/listteas} endpoint \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth} {|  >{\raggedright\arraybackslash}X  |   >{\raggedright\arraybackslash}X |} 
		\hline
		\multicolumn{2}{| >{\raggedright\arraybackslash}X  |}{2. } \\
        \hline
        Request & Response \\
        \hline
        \small
