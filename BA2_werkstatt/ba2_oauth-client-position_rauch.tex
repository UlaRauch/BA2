...
One particular aspect here is the \texttt{logoutSuccessHandler} call that gets a \texttt{ServerLogoutSuccessHandler} Object as an argument. A separate bean had to be written on order to make this work properly, see listing \ref{lst:logoutSuccessHandler}. The \texttt{OidcClientInitiatedServerLogoutSuccessHandler}, which implements the ServerLogoutSuccessHandler interface, takes care of the logout process and calls the Keycloak Server's \texttt{endSessionEndpoint} for this user \cite{HowFixKeycloak2022}, \cite{rozaSpringSecurityOpenID2017}. The \texttt{postLogoutRedirectUri} is the uri that the user will be redirected to after having logged out successfully. User logout is initiated by a \texttt{GET} or \texttt{POST} request to \texttt{{base-url}/logout} as default. The \texttt{/logout} endpoint does not need to be permitted explicitly in the filter chain \cite{rozaSpringSecurityOpenID2017}. 
...
%entfernt, weil es hier zuviel um authentication geht. passiert das wirklich in meinem resource server?
The authorization process when a request for a protected resource comes in, goes like this:
\begin{itemize}
\item A unauthenticated request comes in from the User
\item The \texttt{AuthorizationFilter} throws a \texttt{AccessDeniedException}
...
When the request comes with a bearer token, the \texttt{BearerTokenAuthenticationFilter} extracts the token from the \texttt{HttpServletRequest} and creates a \texttt{BearerTokenAuthenticationToken}, which implements the \texttt{Authentication} interface and represents the authenticated user and contains a \texttt{pricipal}, \texttt{credentials} (can be cleared after authentication to avoid leaking)!!! das passt da nicht wirklich her, ist aber aus der spring doku... and \texttt{authorities}. These are instances of \texttt{GrantedAuthority} and usually represent coarse-grained permission, for example \texttt{role} or \texttt{scope} \cite{ServletAuthenticationArchitecture}. \texttt{authority} claims are not part of the OAuth2 tokens issued by Keyloak (or other common AS), so a authorities converter needs to be written in order to distinguish roles in authorization. 
%keycloak issues tokens with realm roles and client roles ...
