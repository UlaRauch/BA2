...
All Services were build with Spring Boot\footnote{https://spring.io/projects/spring-boot} Version 3.0, an open source application framework for Java. It is an extension of Spring, which provides dependency injection and different modules, like Spring Security, Spring Test or Spring ORM (object-relational mapping).

Spring Boot was created on top of Spring in order to simplify the development of Spring-based applications. It provides default configurations and reduces the need for boilerplate code. One big advantage when using Spring boot is therefore the quick project setup.

Spring Boot projects can be created with the Spring Boot Initializr [!!!link...] which is also available when creating a new Project in IntelliJ. All Maven Dependencies that are needed for a project can be chosen in the beginning, or they can be added later to the \texttt{pom.xml} file.
