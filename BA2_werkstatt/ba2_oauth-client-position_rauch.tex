...
\section{Load testing with JMeter - WIP}
...
For testing purposes, only the minimal implementation without role-mapping was used. Test requests were sent to the gateway's \texttt{/hellotea/{name}} endpoint, which forwards the request to the Tea service's \texttt{/teas/hello/{name}} endpoint and routes the response back from the Tea service to the user agent. Both endpoints were protected except from the unprotected version, so that three numbers could be compared: the response time of requests over the gateway as client to the resource server and back to the client, the response time of requests over the gateway as resource serer to the second resource server and back, and the response time from the unprotected gateway to the unprotected resource server and back. For authentication in the first case, user login was first triggered via the browser so that the client would send a session cookie to the browser and store the user's access token. This session cookie was extracted and sent with each request to the client-gateway. In the second case, the gateway as resource server expects an access token, so an access token was generated with postman and sent with each request.

Testing parameters were the number of threads (which simulate the number of users), \textit{ramp-up period}, which is the time JMeter takes until all the threads are started, loop count, which is the number of loops (requests) that each thread will perform and thread lifetime. The tests were performed in steps for 100 up to 600 concurrent users where 100 users were added to each round.

%!!! welche parameter sinds jetzt wirklich geworden?

The JMeter testscripts were prepared in the JMeter GUI and a Powershell script was created to start the tests in non-gui mode with the necessary parameters.
...
\section{Response times - FEHLT} % raufverschieben zu implementation
...
Additionally, logout functionality was implemented, which is not defined in OAuth2 and only mentioned as optional in the OIDC core specification, therefore adherence to the specific specifications \cite{!!! https://openid.net/specs/openid-connect-session-1_0.html zotero hat gestreikt} was not analyzed. This could be a possible extension of this research.
