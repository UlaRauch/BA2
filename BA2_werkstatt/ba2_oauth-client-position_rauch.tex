...
Ich bedanke mich bei allen Personen, die mich durch dieses Studium begleitet und unterstützt haben. bla...
...
For this purpose the \texttt{spring.security.oauth2.client.registration} base property prefix is used, followed by the registration ID that will be used by Spring Security's \texttt{OAuth2ClientProperties} class. In this project the client's registration ID is \texttt{keycloak- \linebreak gateway-client}. As explained in section \ref{sub:oidc}, \texttt{oidc} must be included in the scope claim. Further, the \texttt{client-id} and the \texttt{client-secret}, as well as the \texttt{authorization-grant linebreak -type} and the \texttt{redirect-uri} are specified. The \texttt{redirect-uri} is the address that the authorization server will send to the user agent to redirect the user back to the application after authorization has been granted (see section \ref{sub:oauth2}).
