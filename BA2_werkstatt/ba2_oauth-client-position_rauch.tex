...
An example for a decoded access token is shown in figure \ref{fig:keycloak-access-token}, although it does not conform entirely to the specifications, as the  \texttt{typ} header parameter contains \texttt{JWT} instead of the required \texttt{at+jwt} value and the \texttt{aud} claim as well as the \texttt{client\_id} claim are missing. The client id is instead present with the \texttt{azp} claim.
...
All implementations and testing were carried out on a Lenovo Thinkpad T490s with a Intel\textregistered Core\texttrademark  i78665U CPU, 1.9 GHz Base Frequency, 4,8 GHz Max Turbo Frequency and 4 Cores. The operating system was Microsoft Windows 11 Pro.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=\linewidth]{images/keycloak-access-token}
...
The protocol is HTTP 1.1 and host is localhost where not defined differently.

We can see that the steps of the authorization code flow match with the specifications. The authorization code is passed through from the AZ to the client, authorization request, token request and token response are present and the access token is passed on from the client to the resource server as bearer token in the \texttt{Authentication} header.

When taking a closer look at the parameters in the token request and response however, ...


Postman:
successful list of teas
userinfo endpoint
access token (claims!)
id token

ohne scope parameter -> default parameter muss konfiguriert werden, z.B. mit role gemappt
scope validierung muss extra implementiert werden
was passiert mit userinfo?
...
\texttt{typ} header parameter has \texttt{JWT} as value instead of \texttt{at+jwt} or \texttt{application/at+jwt} as required in RFC 9068 \cite{bertocciJSONWebToken2021}. The RFC specifies further that resource servers must reject tokens containing any other value. This is not the case with the default Spring Security Configuration.

aud claim is missing
"If the request does not include a "resource" parameter, the authorization server use a default resource indicator in the "aud" claim" bertocciJSONWebToken2021

Token validation with Spring Security can be configured with the \texttt{OAuth2TokenValidator} API.

userinfo endpoint returns only the sub claim. keycloak documentation refers to oidc specification.
config möglich über client scopes -> scope anklicken -> mappers -> add to id token, add to access token, add to userinfo...
+ assign to role: client scopes -> scope anklicken -> scope -> assign to role
ist ein bissl verwirrend und keine doku zu finden.
...
future work:
- best practice, not only poc, e.g. better client authentication (signed jwt...), mTLS, etc.
- testing
