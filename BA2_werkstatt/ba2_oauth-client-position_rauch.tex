...
As already mentioned in chapter \ref{chap:intro}, OAuth2 requires an AZ that issues access tokens, OIDC requires an OP that handles user login. The Keycloak server acts as both and is therefore relevant for the functioning of OAuth2 and OIDC. A description of the Keycloak server and its configuration is given in section \ref{sub:keycloak}.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth} {|  >{\raggedright\arraybackslash}X |} 
        ...
        \hline
        Gateway refers user to its own \texttt{/listteas} endpoint a second time \\
        \hline
    \end{tabularx}
\end{table}
