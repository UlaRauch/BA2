...
% --- Begin of Thesis ----------------------------------------------------
\mainmatter
 \chapter{Introduction - FORSCHUNGSFRAGE ETC. FEHLT NOCH}
\label{chap:intro}
...
Als Standard für Authorisierung in MSA wird vielfach OAuth 2.0 (OAuth2) eingesetzt, das in RFC 6749 und 6750 definiert ist. Allerdings ist damit eine hohe anzahl an weiteren Dokumenten verknüpft, die im Lauf der Jahre dazugekommen sind. Es kann für Developer eine Herausforderung darstellen, hier den Überblick zu behalten \cite{pareckiItTimeOAuth2019}. Seit kurzem gibt es die langersehnte Specification für OAuth 2.1. %Was hat das mit dem Wert meiner arbeit zu tun? ich vergleiche die specifications mit der implementierung!

MSA sind immer noch im Trend, bringen aber Security Challenges mit sich, Security muss unbedingt berücksichtigt werden wegen der großen Schäden, die sonst entstehen können, Auth steht dabei ganz oben auf der Liste, genauere Differenzierung bei Access Control ist sinnvoll zur Schadensbegrenzung, mindestens coarse-grained, z.B. mithilfe von roles/RBAC.
    OAuth2/OIDC ist zwar quasi Industriestandard für Auth in MSA, stellt aber für viele Developer eine Challenge dar, weil es so kompliziert ist, bzw. so viele Specs
    Spring Boot ist gut geeignet für MSA weil man damit recht unaufwändig Applications erstellen kann und Keycloak ist frei verfügbar. Beides bisher gut miteinander kombinierbar dank Keycloak Adapter, der jetzt deprecated ist
    OAuth2-Rollen sind Client, Resource Owner und Resource Server. MSA mit Gateway hat mehr eigenständige Aktoren als OAuth2-Rollen, daher mehrere Interpretationen für Gateway-Rolle möglich.

Daraus ergeben sich die Forschungsfragen:

    Wie kann man OAuth2/OIDC mit Spring Boot 3 und Keycloak ohne Keycloak-Adapter implementieren?
    Wie kann man dabei Userrollen berücksichtigen (bzw. RBAC)?
    Welche Unterschiede ergeben sich aus verschiedenen Interpretationen der OAuth2-Rollenverteilung in MSA für die Performance? -> Ausgewählt zwei Szenarien: Gateway als Client + Gateway als Resource Server

Methodik:

    Frage 1: Prototyp-Implementierung + Analyse, ob OAuth2-Specifications erfüllt werden. Dabei Gateway als Client, weil frontend-application out of scope ist (kann nicht auf wissenschaftliche Quellen, aber zumindest auf Tutorials verweisen, wo das auch gemacht wird).
    Frage 2: Messen der Performance beider Varianten mit JMeter + ungesicherter Variante für Relativierung




Relevanz:
Allgemein: es wird immer noch bemängelt, dass microservice security ein vernachlässigtes kapitel ist, siehe die Überblickspapers aus BA1
Keycloak Adapter deprecated, daher neue Anforderungen an die Configuration mit Spring Security
Es gibt vorschläge für gateway als client (ch4mpys "BFFs"), wo noch? nix akademisches gefunden.
...
%rbac -> gegen confused deputy etc. (nehme et al)	
