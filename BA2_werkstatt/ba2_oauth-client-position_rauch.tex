...
The Gateway for the Teapot project was implemented using the \linebreak\texttt{spring-cloud-starter-gateway} module. Maven dependencies are injected in the \linebreak\texttt{pom.xml} file in the following way:
...
Here it is important to choose the correct starter module and to not get confused by the different oauth2-client dependencies available. This module is intended for the use with spring boot. Then, after having created the client in Keycloak (see section \ref{sub:keycloak}), the application needs to be configured to use these client properties and connect to the authorization server. For this purpose we use the \linebreak\texttt{spring.security.oauth2.client.registration} base property prefix, followed by the registration id that will be used by Spring Security's \texttt{ClientRegistration} class. In this project the client's id is \texttt{keycloak-gateway-client}. As explained in section \ref{subsec:oidc}, \texttt{oidc} must be included as scope. Further, the \texttt{client-id} and the \texttt{client-secret}, as well as the \texttt{authorization-grant-type} and the  \texttt{redirect-uri} are specified. The \texttt{redirect-uri} is the address that the authorization server will send to the user agent to redirect the user back to the application after authorization has been granted (see section \ref{sec:oauth2}).
%weiter mit provider...
